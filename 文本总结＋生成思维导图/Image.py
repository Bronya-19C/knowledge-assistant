...
class InteractiveMindMap:
    ...
    def setup_ui(self):
        ...
        refresh_btn = tk.Button(control_frame, text="刷新", command=self.draw_mindmap,
                               bg='#4ecdc4', fg='white', font=('Arial', 10))
        refresh_btn.pack(side=tk.LEFT, padx=5)
        
        expand_btn = tk.Button(control_frame, text="全部展开", command=self.expand_all,
                              bg='#45b7d1', fg='white', font=('Arial', 10))
        expand_btn.pack(side=tk.LEFT, padx=5)
        
        collapse_btn = tk.Button(control_frame, text="全部收缩", command=self.collapse_all,
        ...
        save_hd_btn = tk.Button(control_frame, text="高清保存", command=self.save_hd_image,
                               bg='#8e44ad', fg='white', font=('Arial', 10))
        save_hd_btn.pack(side=tk.LEFT, padx=5)
    ...
    def save_image(self):
        ...
        try:
            ...
            if file_path:
                ...
                messagebox.showinfo("保存成功", f"思维导图已保存到:\n{file_path}")
    ...
    def save_hd_image(self):
        ...
        try:
            ...
            if file_path:
                ...
                messagebox.showinfo("保存成功", f"高清思维导图已保存到:\n{file_path}")
                
        except Exception as e:
            messagebox.showerror("保存失败", f"保存高清图片时出错:\n{str(e)}")
...
if __name__ == "__main__":
    ...
    #     "列表和元组", "字典和集合", "数值运算",
    #     "正则表达式", "字符编码",
    #     "f-string", "format方法", "百分号格式化",
    #     "继承与多态", "封装与抽象", "设计模式",
    #     "类的定义", "实例化", "属性和方法",
    #     "os模块", "sys模块", "datetime模块", "json模块",
    #     "NumPy", "Pandas", "Matplotlib", "Requests",
    #     "Web开发", "数据分析", "机器学习", "自动化脚本"
    # ]
    
    # for node in leaf_nodes:
    #     if node not in sample_data:
    #         # 根据名称推断层级
    #         if node in ["控制流语句", "函数定义", "异常处理"]:
    #             level = 2
    #         elif node in ["列表和元组", "字典和集合", "数值运算"]:
    #             level = 3
    #         elif node in ["正则表达式", "字符编码"]:
    #             level = 4
    #         elif node in ["f-string", "format方法", "百分号格式化"]:
    #             level = 5
    #         elif node in ["继承与多态", "封装与抽象", "设计模式"]:
    #             level = 2
    #         elif node in ["类的定义", "实例化", "属性和方法"]:
    #             level = 3
    #         else:
    #             level = 2  # 默认层级
    ...
    app = InteractiveMindMap(sample_data)
    app.run()